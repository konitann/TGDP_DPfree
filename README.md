...
### CKKSのノイズによるプライバシ強度の測定
1.  **事前学習（プレラン）による固定クリッピング閾値の決定 (`main_training.py`)**
    * 本番の学習を開始する前に、平文で数イテレーションのダミー学習（プレラン）を実行します。
    * そこで観測された勾配ノルムの中央値を、感度の上限を示す固定のクリッピング閾値 $C$ として設定し、理論計算の安定性を担保しています。
2.  **実測ノイズ（$\sigma_{agg}$）の動的抽出と移動平均**
    * 各イテレーションにおいて、「平文で計算した理想的な勾配」と「Goサーバー（Lattigo）で暗号計算され復号された勾配」の差分を取り、その標準偏差からノイズ量 $\sigma_{agg}$ を実測しています。
    * 極小の浮動小数点演算による微小なブレ（$\epsilon$ の乱高下）を抑えるため、過去数回分のノイズの**移動平均（Moving Average）**を導入して平滑化を行っています。
3.  **アンサンブル評価用スクリプト (`run_experiments_average.py`)**
    * 単発の実測では発生しやすい計算機のランダムなブレを相殺するため、実験を自動で複数回（例：1000回）繰り返し実行し、平均化された $\epsilon$ の推移を対数スケールのグラフとして出力するラッパースクリプトを追加しました。
...
| :--- | :--- | :--- | :--- |
| 特大 | 15 | `[]int{35, 20, 20, 20, 20, 20, 20, 20}` | 論文の最小評価値。ノイズの影響が顕著。 LogNは12|
| 大 | 20 | `[]int{30, 22, 22, 22, 22, 22, 22, 22},` | 近似誤差が明確に観測可能。 |
